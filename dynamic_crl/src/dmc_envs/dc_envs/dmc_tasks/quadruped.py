...
def walk_context(
    ...
    task = Move(desired_speed=_WALK_SPEED, random=random,
                is_evaluation=is_evaluation, easy_init=easy_init)
...
## NEW TASKS
@SUITE.add()  # name becomes "stay_still_context"
def stay_still_context(
    context: Context = {},
    time_limit: int = _DEFAULT_TIME_LIMIT,
    random: Union[np.random.RandomState, int, None] = None,
    environment_kwargs: Optional[Dict] = None,
) -> dm_env:
    """Returns the Stay-Still task (upright reward only) with adapted context."""
    # floor size can be same as walk
    xml_string = make_model(floor_size=_DEFAULT_TIME_LIMIT * _WALK_SPEED)
    if context != {}:
        xml_string = adapt_context(xml_string=xml_string, context=context)
    physics = Physics.from_xml_string(xml_string, common.ASSETS)
...
class Move(base.Task):
    ...
    def initialize_episode(self, physics: Physics) -> None:
        ...
        if self._easy_init:
            # Start upright, at zero speed.
            tiny_rand = 1e-2 * self.random.randn(3)
            orientation = np.array((1, tiny_rand[0], tiny_rand[1], tiny_rand[2]))
    ...
    def get_reward(self, physics: Physics) -> np.float64:
        ...
        if self._is_evaluation and self._eval_log is not None:
            self._eval_log["move"].append(move_reward)
            self._eval_log["upright"].append(_upright_reward(physics))
            self._eval_log["vx"].append(physics.torso_velocity()[0])
            self._eval_log["upright_raw"].append(physics.torso_upright())

        return _upright_reward(physics) * move_reward
    
    def get_termination(self, physics):
        termination_condition = super().get_termination(physics)
        # TODO: consider other termination conditions, e.g. flying off
