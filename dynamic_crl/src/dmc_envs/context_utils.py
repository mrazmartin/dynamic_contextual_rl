...
from dm_control.mujoco import Physics as mj_Physics

import dm_control.mujoco as mujoco

# this is just for static episodical context (not dynamic)
def adapt_context_from_physics(physics: mj_Physics, context: Context) -> None:
    model = physics.model
    ...
    # --- Gravity update ---
    if "gravity_z" in context:
        model.opt.gravity[2] = float(context["gravity_z"])
    
    if "payload_mass" in context:
        new_mass = float(context["payload_mass"])
        m0 = float(physics.named.model.body_mass["payload"])
        # if m0==0, avoid division by zero; treat scale as 1.0
        scale = new_mass / m0 if m0 > 0 else 1.0
        with physics.reset_context():
            physics.named.model.body_mass["payload"]    = new_mass
            physics.named.model.body_inertia["payload"] *= scale

    # Recompute derived quantities -> important!
    physics.forward()

def adapt_context_from_xml(xml_string: bytes, context: Context) -> bytes:
